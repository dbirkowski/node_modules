(function (module) {
    ...
    function fetchRaw(fetchResponse) {
        return fetchResponse._decode().then(function () {
            return fetchResponse._raw;
        });
    }
    ...
    function processDirectoryResult(path, dirResult, targetOnly) {
        ...
        responseItems.forEach(function (responseItem) {
            ...
            //var filename = processDirectoryResultFilename(path, processXMLStringValue(responseBro.iCanHaz1("d:href", "D:href"))).trim()
            var filename = responseBro.iCanHaz1("d:href", "D:href") //.trim()
                
                , resourceType = processXMLStringValue(propsBro.iCanHaz1("d:resourcetype", "D:resourcetype"))
                , itemType = (resourceType.indexOf("d:collection") >= 0 || resourceType.indexOf("D:collection") >= 0) ? "directory" : "file";
    ...
    module.exports = {
        ...
        , old_getDir: function (auth, path) {
            ...
            }).then(handleResponseError).then(function (res) {
                return res.text();
            }).then(function (body) {
        ...
        , getFile: function (auth, path, encoding) {
            ...
            path = removePathPrefixSlash(path);
            return fetch(auth.url + path).then(handleResponseError).then(function (res) {
                if (encoding === "utf8") {
                    return res.text();
                }
                return fetchRaw(res);
        ...
        , putFile: function (auth, path, data, encoding) {
            ...
            return new Promise(function (resolve, reject) {
                var request = http.request({
                    ...
                    if (statusCode >= 400) {
                        var err = new Error("Server returned error: " + statusCode + " " + STATUS_CODES[statusCode]);
